...
def ParasHandler(parseRe):
    ...
    if not (parseRe.d or parseRe.dd):
        print("No debuging infomation to output in this mode...")
    ...
    if parseRe.d:
        ...
        if address is None or fileName is None:
            print('Error. We need 2 paras [--addr] [--filename] in this mode.')
            exit(-1)
        _mode = 2
    ...
    if parseRe.dd:
        ...
        if not (address is not None and fileName is None):
            print('Error. We only accept 1 paras [--addr] in this mode.')
            exit(-1)
        _mode = 3

    if _mode == 2:
        hostsFromFile = FileToDict(fileName)
    else:
        hostsFromFile = FileToDict('dnsrelay.txt')
